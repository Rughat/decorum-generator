...
RSpec.describe ComputedRule::RelativeCountOfColor do
  ...
  describe ".build" do
    ...
    it "randomly builds a rule from the given house when there are more in the first section" do
      ...
      expect(subject.text).to eq("The top floor<span class=\"icon-top-floor\"><\/span> must contain more warm (<span class=\"red\">red<\/span> or <span class=\"yellow\">yellow<\/span>) features than the bottom floor<span class=\"icon-bottom-floor\"><\/span> (as objects and/or wall colors)")
    end
    ...
    it "randomly builds a rule from the given house when there are fewer in the first section" do
      ...
      subject = described_class.build(house: house, feature: feature.color, sections: sections)
      expect(subject).to be_a(described_class)
      expect(subject.text).to eq("The top floor<span class=\"icon-top-floor\"><\/span> must contain fewer warm (<span class=\"red\">red<\/span> or <span class=\"yellow\">yellow<\/span>) features than the bottom floor<span class=\"icon-bottom-floor\"><\/span> (as objects and/or wall colors)")
    end
    ...
    it "randomly builds a rule from the given house when there are an equal amount in both sections" do
      ...
      subject = described_class.build(house: house, feature: feature.color, sections: sections)
      expect(subject).to be_a(described_class)
      expect(subject.text).to eq("The top floor<span class=\"icon-top-floor\"><\/span> must contain an equal amount of warm (<span class=\"red\">red<\/span> or <span class=\"yellow\">yellow<\/span>) features (as objects and/or wall colors) as the bottom floor<span class=\"icon-bottom-floor\"><\/span>")
    end
  end
end
