  ...
  class ComparableCountOfColorVsStyle < Requirement
    ...
    def self.build(house:, sections: Section, feature:)
      ...
      rule.text = if style_count == color_count
        "The #{section.name} must contain an equal number of #{style.display} objects and #{color.display} features (as objects and/or wall colors)".html_safe
      else
        "The #{section.name} must contain #{(style_count < color_count) ? "fewer" : "more"} #{style.display} objects than #{color.display} features (as objects and/or wall colors)".html_safe
      end
      rule.save
      rule
    end
